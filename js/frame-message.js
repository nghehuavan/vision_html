...
function finalStep() {
  ...
  window.localStorage.setItem('message-image-final', imgFinal);
  window.location.href = './design-message-result.html'
}
  
function renderModal() {
  let html = ''
  dataMessageFrame.categoryAndMessage.forEach((element, index) => {
    ...
    element.messages.forEach(ele => {
      html += `<div class="list-frame-message-item" onClick="getImage(${ele.id})">
                <img src="${ele.img}" alt=""/>
              </div>`
    })
    html += `</div>`
...
function onLoadFrameMessage(){
  renderModal()
  checkAge()
  newdrawCanvas.setFrame(5);
  newdrawCanvas.draw()
}
